...
<template><article class="schedule-card" :style="{'--branch':branch?.colorHex||'#64748b'}"><div class="accent"></div><div class="card-main"><div class="card-top"><strong>{{schedule.taskIcon}} {{schedule.taskName}}</strong><BranchBadge :branch="branch"/></div><div class="meta"><span v-if="schedule.startTime">🕒 {{schedule.startTime}}<template v-if="schedule.endTime">–{{schedule.endTime}}</template></span><span>👤 {{schedule.assignees.map(a=>a.label).join(', ')||'Chưa phân công'}}</span></div><div class="actions" v-if="!readonly&&schedule.status!=='COMPLETED'"><button class="success" @click="$emit('complete',schedule)">✓ Hoàn thành</button></div></div></article></template>
